...
def scores_for_possible(dice_set:list) -> list:
    "Returns a list of scores only for the possible sets"
    sets = possible_sets(dice_set)
    score_summary = summarize_potential_scores(dice_set)
    for set in list(score_summary.keys()):
        if set not in sets:
            del score_summary[set]

    return list(score_summary.values())        



def get_max_potential_score_set(dice_set:list) -> tuple:
    "Returns a tuple with the name of the set that returns a max score for a dice set and that max score"
    scores_summary = summarize_potential_scores(dice_set)
    sets = possible_sets(dice_set)
